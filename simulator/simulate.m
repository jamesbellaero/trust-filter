...
for i = 1:nt
    time = time_list(i);
    % Call propagate for all entities
    for j = 1:ne
        ...
        entity_list(j).propagate(time,entity_list);
    end
    ...
    for sensing_entity = entity_list
        for j = 1:ne
            if(sensing_entity == entity_list(j))
                continue
            end
            measurements = sensing_entity.make_measurements(time,entity_list(j));
